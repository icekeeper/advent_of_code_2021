...
private sealed class Number(var parent: Pair?) {
    ...
    fun children(): Sequence<Number> = sequence {
        when (this@Number) {
            is RegularNumber -> yield(this@Number)
            is Pair -> {
                yieldAll(this@Number.left.children())
                yield(this@Number)
                yieldAll(this@Number.right.children())
            }
        }
    }
    ...
    fun reduce(): Number {
        ...
        do {
            reduced = false
            children().filterIsInstance<Pair>().find { it.level() == 4 }?.let {
                reduced = true
                it.explode()
            }
            if (!reduced) {
                children().filterIsInstance<RegularNumber>().find { it.value >= 10 }?.let {
                    reduced = true
                    it.split()
                }
            }
